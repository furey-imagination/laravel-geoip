...
class GeoIP {
    ...
    private function locate_maxmind($ip)
    {
        ...
        if($settings['type'] === 'web_service') {
            ...
            // Set our defaults.
            $defaults = array(
                'path' => app_path().'/database/maxmind/',
                'name' => 'GeoLite2-City.mmdb'
            );
            // Validate everything (for backwards compatibility).
            if (!isset($settings['database'])) {
                $settings['database'] = [];
            }
            $path = isset($settings['database']['path']) && !empty($settings['database']['path']) ? $settings['database']['path'] : $defaults['path'];
            $name = isset($settings['database']['name']) && !empty($settings['database']['name']) ? $settings['database']['name'] : $defaults['name'];
            // If the database can't be found, return the default location.
            if (!file_exists($path.$name)) {
                return $this->default_location;
            }
            // Boot the database.
            $maxmind = new Reader($path.$name);
        }
